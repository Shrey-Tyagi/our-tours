...
function App() {
  ...
  const [tourDetailsVal, setOurDetailsVal] = useState(tourDetails);
  useEffect(() => {
    fetch('https://course-api.com/react-tours-project')
      ...
      .then((data) => {
        setOurDetailsVal(data);
        setInitialLoad(false);
      })
      .catch((response) => {
        // Setting static values
        setTimeout(() => {
          setOurDetailsVal(tourDetails);
          setInitialLoad(false);
        }, 1000);
  ...
  return (
    <div className="app">
      ...
      <div className="tours">
        {tourDetailsVal.map((tour) => {
          return (
            <Tour
              key={tour.id}
              tourDetails={tour}
              setOurDetailsVal={setOurDetailsVal}
            />
          );
        })}
      </div>
...
function Tour(props) {
  const { image, name, price, info } = props.tourDetails;
  const [btnValue, setBtnValue] = useState('Read more!');
  const [infoVal, setInfoVal] = useState(info.slice(0, 200) + '...');

  const toggleInfo = () => {
    if (btnValue === 'Read more!') {
      setInfoVal(info);
      setBtnValue('Show less');
    } else {
      setInfoVal(info.slice(0, 200) + '...');
      setBtnValue('Read more!');
    }
  };

  const handleNotInterestedClick = (tour) => {
    props.setOurDetailsVal((tourDetailsVal) => {
      return tourDetailsVal.filter((t) => t !== tour);
    });
  };
  ...
  return (
    <div className="tour">
      ...
      <div className="tour-info">
        ...
        <h4>
          {infoVal}
          <button className="show-hide" onClick={toggleInfo}>
            {btnValue}
          </button>
          <button
            className="delete-tour"
            onClick={() => {
              handleNotInterestedClick(props.tourDetails);
            }}
          >
            Not Interested
          </button>
